...
public class MenuPageSauceAdapter extends BaseExpandableListAdapter {
    ...
    public MenuPageSauceAdapter(Context context,String heading, ArrayList<dtoMyOrder> myOrderArrayList) {
    ...
    public View getChildView(final int groupPosition, final int childPosition, final boolean isLastChild, final View convertView, final ViewGroup parent) {
        ...
        if (myOrder.getItemCount()==0){
            textView_Counter.setText("");
        }else {
            textView_Counter.setText("X"+ myOrder.getItemCount());
        }
        ...
        imageView_Plus.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (Constants.GroupPosition==-1 && Constants.ChildPosition==-1){
                    ...
                    textView_Counter.setText("X"+ Constants.Counter);
                    myOrderArrayList.get(childPosition).setItemCount(Constants.Counter);

                    Events.AdapterToActivity adapterToActivity = new Events.AdapterToActivity(childPosition);
                    GlobalBus.getBus().post(adapterToActivity);

                }
                else {
                    myOrderArrayList.get(Constants.ChildPosition).setItemCount(0);

                    Events.counterDecrement counterDecrement = new Events.counterDecrement(Constants.ChildPosition);
                    GlobalBus.getBus().post(counterDecrement);
...
//                    textView_Counter.setVisibility(View.GONE);
                    ...
                    Events.counterDecrement counterDecrement = new Events.counterDecrement(childPosition);
                    GlobalBus.getBus().post(counterDecrement);
                }
            }
        });
